...
<body class="">
      ...
      <!-- 主体部分 -->
       ...
       <div style="padding: 15px;">
           ...
           <tr v-for="(message,index) in messages">
            ...
            <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">{{message.msgContent}}</td>
             <td>
	...
	var app = new Vue({
		...
		methods:{
		    load:function(){
				$.post("/TxCourse/studentCourse/getMyMessage",function (result) {
					if(result.status != 0)
						layer.msg("暂时没有数据哦！");
					else
					{
	        ...
	        messageDetail:function(message){
	        	  $.ajax({
	                    url:"/TxCourse/studentCourse/getMessage",
	                    dataType:"json",
	                    type:"POST",
	                    data:{id:message.id},
	                    success:function(result){
	                    	if(result.status==0) {
	                    		$.ajax({
	                    			url:"/TxCourse/studentCourse/messageReaded",
	                    			dataType:"JSON",
	                    			type:"POST",
	                    			data:{id:message.id},
	                    			beforeSend:function(){
